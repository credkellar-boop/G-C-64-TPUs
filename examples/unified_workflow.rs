...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let web_tracker = ai_mode::SearchAgent::new("monitor open-source repository updates");
    ...
    spark_agent.on_alert(web_tracker, |alert_data| async {
        let visual_report = flow::generate_image(&alert_data, "Gemini Omni").await?;
        println!("Generated report using Flow credits. Saved to: {}", visual_report.path);
    });
